...
- Create a **slides section**: `content/slides/` with `_index.md` (see Output formats below).
- Each presentation is a **page bundle**: `content/slides/<slug>/` containing:
  - `index.md` — frontmatter and optional body text
  - `slides.pdf` — the presentation PDF (must be named `slides.pdf`)

The processing script writes **one WebP per slide** to `assets/slides/<slug>/slide-001.webp`, `slide-002.webp`, etc. (highest resolution, typically 1920px width cap). The theme uses Hugo’s image processing (`resources.Get` + `Resize "400x webp q100"` etc.) to generate thumb, medium, and full sizes at build time. No pre-generated multi-tier files are required in the repo. For backwards compatibility, if assets are missing, the theme falls back to legacy bundle images at `content/slides/<slug>/slides/slide-NNN-{thumb,medium,full}.webp`.
...
slides:
  pdf: "slides.pdf"      # Filename in bundle (always slides.pdf)
...
Slides are not rendered from PDF in the browser. You must generate **one high-res WebP per slide** from each `slides.pdf` using the theme script:

**Script:** `themes/hugo-techie-personal/scripts/process-slides.sh`

**Usage:**

```bash
# From site root; processes all bundles under content/slides/
./themes/hugo-techie-personal/scripts/process-slides.sh content/slides
```

The script:

1. Scans `content/slides/*/` for `slides.pdf`
2. Extracts pages with `pdftoppm` (poppler-utils) at 300 DPI
3. Writes **one WebP per slide** (quality 100, width capped at 1920px) to **`assets/slides/<slug>/slide-NNN.webp`**
4. Writes `content/slides/<slug>/slides/metadata.json` with `page_count`, `source_hash`, and dimensions (used for skip-by-hash and frontmatter update)
5. Updates `page_count` in `index.md` frontmatter
