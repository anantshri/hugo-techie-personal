...
The theme supports hosting presentation slides as page bundles. PDFs are converted to multi-resolution images (thumb, medium, full) and displayed in an interactive viewer with keyboard navigation, grid overview, and fullscreen. No in-browser PDF rendering is used.
...
The theme expects slide images at `content/slides/<slug>/slides/slide-001-thumb.webp`, `slide-001-medium.webp`, `slide-001-full.webp` (and JPEG fallbacks). These are generated by the processing script; do not create them manually.
...
- Omit `compact` or set `compact="false"` for the full interactive viewer (toolbar, prev/next, grid, fullscreen, download).

**Card only (compact):**

```hugo
{{< slideviewer src="/slides/presentation-slug/" compact="true" >}}
```

Renders a linked card with thumbnail and metadata; no viewer.
...
Slides are not rendered from PDF in the browser. You must generate images from each `slides.pdf` using the theme script:
...
3. Generates three resolution tiers with ImageMagick: thumb (400px), medium (1024px), full (1920px)
4. Creates WebP (via `cwebp`) and JPEG for each tier
5. Writes `slides/metadata.json` with `page_count`, `source_hash`, and dimensions
6. Optionally updates `page_count` in `index.md` frontmatter
